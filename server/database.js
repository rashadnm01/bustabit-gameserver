      ...
      client.query(query, params, function (err, result) {
        ...
        if (err) {
          if (err.code === "40P01" || err.sqlState === "40P01") {
            console.log(
              "Warning: Retrying deadlocked transaction: ",
              query,
              params
            );
            return doIt();
...
exports.getLastGameInfo = function (callback) {
  query("SELECT MAX(id) id FROM games", function (err, results) {
    ...
    query(
      "SELECT hash FROM game_hashes WHERE game_id = $1",
      [id],
...
exports.createGame = function (gameId, callback) {
  ...
  query(
    ...
    function (err, results) {
      ...
      query(
        "INSERT INTO games(id, game_crash) VALUES($1, $2)",
        [gameId, gameCrash],
        function (err) {
          if (err) return callback(err);
